...
export class CustomerCareController {
  ...
  @Get('health') @Perm(CustomerCarePermissions.CHANNEL_READ) health() {
  ...
  @Get('channels') @Perm(CustomerCarePermissions.CHANNEL_READ) channels() {
  ...
  @Post('conversations')
  @Perm(CustomerCarePermissions.CONVERSATION_WRITE)
  ...
  @Perm(CustomerCarePermissions.ASSIGN)
  ...
  @Put('conversations/:id/team') @Perm(CustomerCarePermissions.ASSIGN) setTeam(
    @Param('id') id: string,
    @Body() dto: TeamDto,
  ) {
  ...
  @Patch('contacts/:id')
  @Perm(CustomerCarePermissions.CONTACT_UPDATE)
  patchContact(
    @Param('id', ParseIntPipe) id: number,
