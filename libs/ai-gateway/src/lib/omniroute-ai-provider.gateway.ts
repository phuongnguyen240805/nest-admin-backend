...
export class OmniRouteAiProviderGateway implements AiProviderGateway {
  async generateText(request: AiTextRequest): Promise<AiTextResult> {
    ...
    const toolCalls = this.extractToolCalls(response.body)
    if (!text.trim() && !toolCalls.length) {
      throw new ServiceUnavailableException('AI gateway returned empty text response')
    }
  ...
  private headers(request: { idempotencyKey?: string; sessionId?: string }): Record<string, string> {
    const apiKey = process.env.AI_GATEWAY_API_KEY ?? process.env.OMNIROUTE_API_KEY
    if (!apiKey) {
      throw new ServiceUnavailableException('AI gateway API key is required')
    }
  ...
  private url(path: string): string {
    const baseUrl = process.env.AI_GATEWAY_BASE_URL ?? process.env.OMNIROUTE_BASE_URL ?? (process.env.NODE_ENV === 'production' ? 'http://9router:20128/v1' : 'http://localhost:20128/v1')
    return `${baseUrl.replace(/\/$/, '')}/${path.replace(/^\//, '')}`
  }


  private toOpenAiMessage(message: AiTextRequest['messages'][number]): Record<string, unknown> {
    return {
      role: message.role,
  ...
  private extractToolCalls(response: OpenAiChatResponse) {
